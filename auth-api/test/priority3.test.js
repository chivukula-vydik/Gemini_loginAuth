...
test('GET /attendance/config returns the configured shift', async () => {
  const emp = await User.create({ email: 'shift-1@x.com', displayName: 'E', roles: ['employee'] });
...
test('GET /attendance/stats uses the configured shift start to determine lateCount', async () => {
  const emp = await User.create({ email: 'shift-2@x.com', displayName: 'E', roles: ['employee'] });
...
test('POST /attendance/checkin handles a concurrent double-tap without a 500', async () => {
  const emp = await User.create({ email: 'race-1@x.com', displayName: 'E', roles: ['employee'] });
...
test('POST /attendance/regularise rejects a second request while one is pending', async () => {
  const emp = await User.create({ email: 'reg-stack@x.com', displayName: 'E', roles: ['employee'] });
...
test('Approving leave over a day the employee already clocked in appends a conflict note instead of overwriting', async () => {
  const pm = await User.create({ email: 'conflict-pm@x.com', displayName: 'PM', roles: ['pm'] });
  const emp = await User.create({ email: 'conflict-emp@x.com', displayName: 'E', roles: ['employee'] });
...
test('DELETE /leave/:id cancels a pending request owned by the caller', async () => {
  const emp = await User.create({ email: 'cancel-1@x.com', displayName: 'E', roles: ['employee'] });
  const applied = await request(app).post('/leave')
    .set('Authorization', bearer(emp))
...
test('DELETE /leave/:id is forbidden for a non-owner', async () => {
  const emp = await User.create({ email: 'cancel-owner@x.com', displayName: 'E', roles: ['employee'] });
  const other = await User.create({ email: 'cancel-other@x.com', displayName: 'O', roles: ['employee'] });
  const applied = await request(app).post('/leave')
    .set('Authorization', bearer(emp))
...
test('DELETE /leave/:id rejects cancelling a rejected request', async () => {
  const pm = await User.create({ email: 'cancel-pm@x.com', displayName: 'PM', roles: ['pm'] });
  const emp = await User.create({ email: 'cancel-decided@x.com', displayName: 'E', roles: ['employee'] });
  const applied = await request(app).post('/leave')
    .set('Authorization', bearer(emp))
    .send({ type: 'casual', startDate: '2026-07-11', endDate: '2026-07-11', reason: 'x' });
  await request(app).patch(`/leave/${applied.body._id}/decide`)
    .set('Authorization', bearer(pm)).send({ decision: 'rejected' });
...
test('DELETE /leave/:id returns 404 for a missing request', async () => {
  const emp = await User.create({ email: 'cancel-404@x.com', displayName: 'E', roles: ['employee'] });
