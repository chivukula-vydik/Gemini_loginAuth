...
- **PM task create/edit** (`web/src/pm/Projects.tsx` task form): a `startDate` date input only. The PM does **not** enter the estimate directly — per the existing product decision ("remove PM estimate input"), estimates come solely from the employee propose → PM approve flow. Sends `{ startDate }` (plus title/assignee/requiredSkills).
- **Employee propose-estimate** (`web/src/pm/MyTasks.tsx`): number input + unit `<select>`; sends `{ value, unit }`.
- On the backend, the task create/edit route accepts `startDate`. `PATCH /tasks/:id/estimate` accepts `value`+`unit` and recomputes the derived hours. `PATCH /tasks/:id/estimate/decision` (approve) copies `proposedValue`/`proposedUnit` → `estimateValue`/`estimateUnit` and recomputes `estimatedHours`.
