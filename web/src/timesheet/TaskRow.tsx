...
export function TaskRow({ task, readOnly = false, onRename, onCellChange, onDelete }: Props) {
  ...
  return (
    <tr>
      ...
      {DAYS.map((d) => (
        <td key={d}>
          <TimeCell minutes={task.entries[d] || 0} readOnly={readOnly} onChange={(m) => onCellChange(d, m)} />
        </td>
      ))}
